...
// FIX: Removed JSX (MeltdownTimer component) from this file
// FIX: Added resetFacility, WebSocket actions, and ws state
...
const STORE_VERSION = 31; // bumped for notifications + ws
...
export interface GameState {
  ...
  facility: {
    ...
    ordealsTriggeredToday: string[];
    pendingOrdeal: any | null;
...
const useGameStore = create<GameState>()(
  persist(
    (set, get) => ({
      ...
      connectToRoom: (roomId: string, identityId?: string) => {
        const state = get();
        if (state.ws && state.ws.readyState === WebSocket.OPEN && state.wsRoomId === roomId) return;
        if (state.ws) {
          state.ws.close();
        }
        const wsUrl = (import.meta.env.VITE_SERVER_URL || 'https://qliphoth-backend.archlouder4.workers.dev')
          .replace(/^https?:\/\//, '');
        const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
        const ws = new WebSocket(`${protocol}//${wsUrl}/room/department/${roomId}`);
        let joinSent = false;

        ws.onopen = () => {
          set({ ws, wsConnected: true, wsRoomId: roomId });
          const identity = identityId || state.selectedIdentityId || undefined;
          const deptKey = state.facility.departmentKey;
          if (!joinSent) {
            joinSent = true;
            ws.send(JSON.stringify({
              type: 'join',
              playerId: state.facility.members[0] || crypto.randomUUID(),
              playerName: 'Guest',
              identityId: identity,
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === 'stateUpdate') {
              if (data.facility) {
                set({ facility: data.facility, players: data.players || [], combat: data.combat || null });
              }
            } else if (data.type === 'departmentRoomDisbanded') {
              set({
                facility: {
        ...
        ws.onclose = () => {
          set({ ws: null, wsConnected: false, wsRoomId: null });
        };
      ...
      sendAction: (type: string, payload?: any) => {
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          ...
          console.warn('WebSocket not connected; action not sent:', type);
        }
      },
    ...
    }),
    {
      ...
      migrate: (persistedState: any, version: number) => {
        ...
        const newFields = ['greatestOrdealTime', 'ordealsTriggeredToday', 'pendingOrdeal', 'qliphothMeter', 'qliphothMax', 'meltdownActive', 'meltdownTarget', 'meltdownExpiresAt', 'ordeals', 'safeRoomUnlocked', 'panicCount', 'log'];
